class JM extends ComicSource {
    ...
    categoryComics = {
        ...
        load: async (category, param, options, page) => {
            if (category !== "每週必看") {
                ...
                // 修复：过滤掉解析为 null 的条目
                let comics = data.content.map((e) => this.parseComic(e)).filter(c => c != null)
                return {
                    comics: comics,
                    maxPage: maxPage
                }
                ...
                let data = JSON.parse(res)
                let comics = data.list.map((e) => this.parseComic(e)).filter(c => c != null)
                return {
                    comics: comics,
                    maxPage: 1
                }
        ...
        ranking: {
            ...
            load: async (option, page) => {
                // 内部已调用 categoryComics.load，已有过滤逻辑
    ...
    favorites = {
        ...
        loadComics: async (page, folder) => {
            ...
            // 修复：过滤掉解析为 null 的条目
            let comics = json.list.map((e) => this.parseComic(e)).filter(c => c != null)
            return {
                comics: comics,
                maxPage: maxPage
            }
