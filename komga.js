...
class Komga extends ComicSource {
	...
	get authToken() {
		...
		const encoded = Convert.encodeBase64(Convert.encodeUtf8(combined))
	...
	explore = [
		{
			...
			load: async () => {
				...
				if (!base) throw '请先在设置中配置有效的服务器地址'
	...
	// ===== 修复点：refreshReferenceData 增强错误处理 =====
