class ShonenJumpPlus extends ComicSource {
  ...
  explore = [
    {
      ...
      load: async () => {
        ...
        if (!dailyRankingSection?.dailyRankings) {
          throw "Cannot fetch daily ranking data";
        }

        const dailyRanking = dailyRankingSection.dailyRankings.find(
          (ranking) => ranking.ranking?.__typename === "DailyRanking"
        );

        if (!dailyRanking?.ranking?.items?.edges) {
          throw "Cannot fetch ranking items";
        }
  ...
  search = {
    load: async (keyword, _, page) => {
      ...
      const response = await this.graphqlRequest("SearchResult", { keyword });
  ...
  async graphqlRequest(operationName, variables) {
    ...
    if (response.status !== 200) {
      // 输出响应体前200字符，方便排查
      const preview = response.body.slice(0, 200);
      throw `GraphQL request failed (status ${response.status}): ${preview}`;
    }

    try {
      return JSON.parse(response.body);
    } catch (e) {
      throw `Invalid JSON response: ${response.body.slice(0, 200)}`;
    }
  }
  ...
  replaceCoverUrl(url) {
    return (url || "").replace("{height}", "1500").replace("{width}", "1500") || "";
  }
  ...
  async fetchBearerToken() {
    const response = await Network.post(
      ...
      JSON.stringify({ deviceId: this.deviceId }) // 官方要求传递 deviceId
    );

    if (response.status !== 200) {
      throw `Token request failed with status ${response.status}: ${response.body.slice(0, 200)}`;
    }
  ...
  async fetchEpisodes(id) {
    ...
    return (response?.data?.series?.episodes?.edges || []).map((edge) => edge.node);
  }
